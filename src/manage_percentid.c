...
/*   Updated: 2017/07/23 13:14:43 by bpierce          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (PFT == CC_TYPE || PFT == SS_TYPE ||
			PFT == DD_TYPE || PFT == OO_TYPE || PFT == UU_TYPE)
	{
		PFT = (PFT == CC_TYPE) ? WI_TYPE : PFT;
		PFT = (PFT == SS_TYPE) ? WC_TYPE : PFT;
...
{
	set[0].f = &set_flags;
	set[1].f = &set_fieldwidth;
	set[2].f = &set_precision;
	set[3].f = &set_length;
	set[4].f = &set_type;
}
